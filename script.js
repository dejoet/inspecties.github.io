...
doc.rect(margin, y, pageWidth - 2 * margin, headerHeight, "FD");

// x-centrum en middenlijn van je kader
const tekstX     = margin +3;
const headerMidY  = y + headerHeight / 2;

// 1) “Inspectierapport” in bordeaux (#870930 → 135,9,48)
doc.setFont("Gill Sans MT", "bold");
doc.setFontSize(18);
doc.setTextColor(135, 9, 48);
doc.text(
  "Inspectierapport",
  tekstX,
  headerMidY - 4,           // iets omhoog t.o.v. midden
  { baseline: "middle" }
);

// 2) “Risicoanalyse BRAND” in donkerblauw (vb. #004B86 → 0,75,134)
doc.setFontSize(20);
doc.setTextColor(0, 75, 134);
doc.text(
  "Risicoanalyse | BRAND",
  tekstX,
  headerMidY + 6,           // iets omlaag t.o.v. midden
  { baseline: "middle" }
);

// **HIER** resetten we alle tekst weer naar zwart
doc.setTextColor(0, 0, 0);
doc.setFont("Gill Sans MT", "normal");
doc.setFontSize(11);

// schuif y door tot onder je header
y += headerHeight + 6;



  // ─── INFO KADER ─────────────────────────────────────────────────
  // kader voor je persoonsgegevens
  const infoHeight = 20;
  doc.setDrawColor(0);
  doc.setFillColor(255);           // wit gevuld
  doc.rect(margin, y, pageWidth - 2 * margin, infoHeight);
  // tekstregels
  doc.setFontSize(11);
  doc.setFont("Gill Sans MT", "normal");
  const textX = margin + 3;
  let textY = y + 6;
  doc.text(`Naam: Joetri Van Dijck`, textX, textY);
  doc.text(`Functie: Field Underwriter`, textX, textY + 6);
  doc.text(`E-mail: joetri.van.dijck@vivium.be`, textX, textY + 12);
  y += infoHeight + 8;

  // … hier volgt de rest van je bestaande PDF-generatiecode …

  
  const maatschappij = document.getElementById("maatschappij").value;
  const zetel = document.getElementById("zetel").value;
  const beheerder = document.getElementById("beheerder").value;
  const polisnummer = document.getElementById("polisnummer").value;
  const verzekerde = document.getElementById("verzekerde").value;
  const liggingrisico = document.getElementById("liggingrisico").value;
  const producent = document.getElementById("producent").value;
  const datumbezoek = document.getElementById("datumbezoek").value;
  const datumrapport = document.getElementById("datumrapport").value;
  ...
  const bestanden = document.getElementById("afbeeldingen").files;
    ...
    const gegevens = [
      ...
      ["liggingrisico", liggingrisico],
      ["producent", producent],
      ["datumbezoek", datumbezoek],
      ["Datumrapport", datumrapport],
    ];
